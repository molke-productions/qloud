...
SplPlot::SplPlot(const QString& aDir, IRInfo anIi, QWidget *parent)
    :Plotter(parent)
{
...
bool SplPlot::gnuplotSeries(const QString &filename)
{
    QFile file(filename);
...
void SplPlot::enablePhase(int state) {
    ...
    if(state) {
        appendSeries(phaseCurve, nullptr, 0, YPAxis, Qt::AlignRight);
    } else {
